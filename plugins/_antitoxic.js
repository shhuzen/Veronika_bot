...
user.warn += 1
if (!(user.warn >= 4)) await m.reply(`${user.warn == 1 ? `*@${m.sender.split`@`[0]}*` : `*@${m.sender.split`@`[0]}*`}, Имеешь: (${isToxic}) придуприждения... Имеешь: *${user.warn}/4*\n\nпридуприждения.`, false, { mentions: [m.sender] })}

if (user.warn >= 4) {
user.warn = 0
await m.reply(`'Ты будешь удален  \n*@${m.sender.split`@`[0]}*`, false, { mentions: [m.sender] })
user.banned = true
