let handler = async (m, { conn, text, participants }) => {
  ...
  if (total == 0)
    return conn.reply(
      m.chat,
      `*[❗𝙸𝙽𝙵𝙾❗]* 𝙴𝚂𝚃𝙴 𝙶𝚁𝚄𝙿𝙾 𝙽𝙾 𝚃𝙸𝙴𝙽𝙴 𝙵𝙰𝙽𝚃𝙰𝚂𝙼𝙰𝚂, 𝚀𝚄𝙴 𝙱𝚄𝙴𝙽 𝚃𝚁𝙰𝙱𝙰𝙹𝙾 𝙷𝙰𝙲𝙴 𝙴𝙻 𝙰𝙳𝙼𝙸𝙽`,
      m
    );
  m.reply(
    `[ ⚠ Внимание ⚠ ]\n
    Молчание в группе платное, дайте знак что вы живы 🙈😂
    \Группа: ${await conn.getName(m.chat)} & неактивные: \n${sum}\n \n \n${sider
      .map((v) => "  👻 @" + v.replace(/@.+/, ""))
      .join(
        "\n"
