// Scraper Creado Por Juls Modders
// Grupo Vip Privado
...
class YT {
     ...
     */
    static isYTUrl = (url) => {
        const ytIdRegex = /(?:https?:\/\/(?:www\.)?youtube\.com\/(?:[^\/]+\/\S+\/|(?:v|e(?:mbed)?)\/|\S*?[?&]v=))([a-zA-Z0-9_-]{11})/;
    ...
    static downloadMusic = async (query) => {
        try {
            ...
            const search = getTrack[0]; // Use the first result from search
    ...
    static mp3 = async (url, metadata = {}, autoWriteTags = false) => {
        try {
            ...
            stream.on('progress', (chunkLength, downloaded, total) => {
                ...
                process.stdout.write(`, estimated remaining: ${estimatedDownloadTime.toFixed(2)} minutes`);
            });
            ...
            if (autoWriteTags) await this.WriteTags(songPath, { Title: videoDetails.title, Artist: videoDetails.author.name, Image: videoDetails.thumbnails.slice(-1)[0].url, Album: 'YouTube Download' });

            return { meta: { Title: videoDetails.title, Artist: videoDetails.author.name, Image: videoDetails.thumbnails.slice(-1)[0].url, Album: 'YouTube Download' }, path: songPath };
        } catch (error) {
            throw new Error(error);
        }
    }
