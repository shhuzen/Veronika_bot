...
export async function handler(chatUpdate) {
  ...
  // ДОБАВИТЬ СЮДА ПРОВЕРКУ НА LOCATION СООБЩЕНИЯ
  // Блокировка location сообщений
  for (const msg of chatUpdate.messages) {
    // Проверяем только входящие сообщения (не от самого бота)
    if (msg.key.fromMe) continue;

    // Удаляем location сообщения из чата
    if (msg.message?.locationMessage) {
      try {
        ...
        await this.sendMessage(msg.key.remoteJid, {
          text: "⚠️ Отправка вирусов запрещена в этом чате",
        });

        console.log(`Удалено location сообщение в чате: ${msg.key.remoteJid}`);
        return; // Прерываем дальнейшую обработку этого сообщения
      } catch (error) {
        console.error("Ошибка при удалении location сообщения:", error);
      }
    }
  }
  ...
  try {
    ...
    m.isWABusiness =
      global.conn.authState?.creds?.platform === "smba" ||
      global.conn.authState?.creds?.platform === "smbi";
    m.isChannel =
      m.chat.includes("@newsletter") || m.sender.includes("@newsletter");
    ...
    for (let name in global.plugins) {
      ...
      if ((usedPrefix = (match[0] || "")[0])) {
        ...
        if (
          ...
          let chat = global.db.data.chats[m.chat];
          let user = global.db.data.users[m.sender];
          if (name != "propietario(a)-unbanchat.js" && chat?.isBanned) return;
          if (name != "propietario(a)-unbanuser.js" && user?.banned) return;
          if (name != "grupo-unbanuser.js" && user?.banned) return;
        }
        ...
        if (adminMode && !isOwner && !isROwner && m.isGroup && !isAdmin && gata)
          return;
        ...
        if (xp > 2000) m.reply("Exp limit"); // LÍMITE DE EXP
        else if (
          !isPrems &&
          plugin.money &&
          global.db.data.users[m.sender].money < plugin.money * 1
        ) {
        ...
        if (
          ...
          this.reply(
            m.chat,
            `${lenguajeGB["smsCont7"]()} *${usedPrefix}buy*`,
            m
          );
        ...
        if (plugin.level > _user.level) {
          this.reply(
            m.chat,
            `${lenguajeGB["smsCont9"]()} *${plugin.level}* ${lenguajeGB[
              "smsCont10"
            ]()} *${_user.level}* ${lenguajeGB[
              "smsCont11"
            ]()} *${usedPrefix}nivel*`,
            m
          );
        ...
        if (m.money) m.reply(+m.money + " *COINS USADO(S)*");
        break;
      }
    }
  } catch (e) {
    ...
    if (
      ...
      m.text.match(
        /(привет|как дела|дурак|бот|здраствуйте|🥰|Галатея|tion|age|ous|ate|and|but|ify)/gi
      )
    ) {
      let emot = pickRandom([
        "😀",
            ...
            if (
              ...
              if (numerosPermitidos.some((num) => user.startsWith(num))) {
                this.sendMessage(
                  id,
                  {
                    // text: `*${lenguajeGB["smsAvisoAG"]()}${lenguajeGB[
                    //   "smsInt1"
                    // ]()} @${user.split("@")[0]} ${lenguajeGB["smsInt2"]()}*`,
                    // mentions: [user],
                    text: `*@${user.split("@")[0]} ${lenguajeGB["smsInt2"]()}*`,
                    mentions: [user],
                  },
                  { quoted: null }
                );
                let responseb = await this.groupParticipantsUpdate(
                  id,
                  [user],
                  "remove"
                );
    ...
    if (nk.isGroup == false) {
      if (nk.status == "offer") {
        ...
        let callmsg = await this.reply(
          nk.from,
          lenguajeGB["smsHandlerLlamar"](tagUserL, llamadaVideo),
          false,
          { mentions: [nk.from] }
        );
...
export async function deleteUpdate(message) {
  try {
    ...
    await this.reply(
      msg.chat,
      lenguajeGB["smsAntiEliminar"](userDelete).trim(),
      msg,
        ...
        contextInfo: {
          externalAdReply: {
            ...
            body: [
              wm,
              "😻 𝗦𝘂𝗽𝗲𝗿 " + gt + " 😻",
              "🌟 centerГалатея.gmail.com",
            ].getRandom(),
            thumbnail: gataImg.getRandom(),
            sourceUrl: [md, yt, nna, nn, nnn, ig, paypal, fb].getRandom(),
          },
        },
      },
    },
